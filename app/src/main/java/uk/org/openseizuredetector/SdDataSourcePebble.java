...
public class SdDataSourcePebble extends SdDataSource {
    ...
    public SdDataSourcePebble(Context context, Handler handler, SdDataReceiver sdDataReceiver) {
        ...
        try {
            int prefsId = useSdServerBinding().getResources().getIdentifier(
                    "seizure_detector_prefs", "xml", useSdServerBinding().getPackageName());
            if (prefsId != 0) {
     ...
     * Uses the protected useSdServerBinding() to verify connection.
     */
    public void getPebbleStatus() {
        long tnow = System.currentTimeMillis();
        long tdiff = tnow - mPebbleStatusTime;

        // Use the binding to check hardware state
    ...
    public void startWatchApp() {
        ...
        // Use the handler from the binding to delay the start (Kirk delay)
        new Handler(useSdServerBinding().getMainLooper()).postDelayed(() -> {
    ...
    public void showToast(final String msg) {
        if (useSdServerBinding() != null) {
            new Handler(useSdServerBinding().getMainLooper()).post(() -> {
                Toast.makeText(useSdServerBinding(), msg, Toast.LENGTH_SHORT).show();
            });
        }
    }
